...
public class StatusNestedRepoTest {
    ...
    void nestedReposAppearOnlyInIgnored(@TempDir Path tmp) throws Exception {
        try (VglTestHarness.VglTestRepo repo = VglTestHarness.createRepo(tmp)) {
            ...
            Git git0 = null, git1 = null;
            try {
                git0 = Git.init().setDirectory(nested0.toFile()).call();
                git1 = Git.init().setDirectory(nested1.toFile()).call();
                ...
                if (git0 != null) git0.close();
                if (git1 != null) git1.close();
                // Recursively delete nested repo directories
                java.nio.file.Files.walk(nested0)
                    .sorted(java.util.Comparator.reverseOrder())
                    .forEach(p -> {
                        try { java.nio.file.Files.deleteIfExists(p); } catch (Exception ignored) {}
                    });
                java.nio.file.Files.walk(nested1)
                    .sorted(java.util.Comparator.reverseOrder())
                    .forEach(p -> {
                        try { java.nio.file.Files.deleteIfExists(p); } catch (Exception ignored) {}
                    });
