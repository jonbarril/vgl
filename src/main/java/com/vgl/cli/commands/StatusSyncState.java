    ...
    public static void printSyncState(Git git, String remoteUrl, String remoteBranch, String localBranch, boolean verbose, boolean veryVerbose) {
        ...
        if (!hasRemote) {
            try {
                ...
                System.out.print("(error reading commits)");
            }
        } else {
            try {
                org.eclipse.jgit.lib.ObjectId localHead = git.getRepository().resolve("HEAD");
                ...
                if (localHead == null) {
                    ...
                    // If user requested verbose output, still show local commits so -v/-vv remain informative
                    if (verbose || veryVerbose) {
                        try {
                            System.out.println("  -- Local Commits:");
                            Iterable<org.eclipse.jgit.revwalk.RevCommit> logs = git.log().call();
        ...
        // Intentionally do not emit an extra blank line here — callers will
        // continue printing subsequent sections (FILES) immediately after
        // the COMMITS block. This keeps output compact when no verbosity
        // flags are provided.
    }
}
