...
public class HelpCommand implements Command {
    ...
    private static String generalHelpVerbose() {
        return String.join("\n",
			...
			"  status [-v|-vv] [-context [URL]] [-changes] [-history] [-files]",
            "",
            "Local work:",
            "  track GLOB... | -all",
            "  untrack GLOB... | -all",
            ...
            "",
            "Review:",
            "  diff  [-v|-vv] [-all] [SOURCE] [SOURCE] [GLOB...]",
            "  log [-v|-vv] [-all] [-graph] [COMMIT..COMMIT] [DATE..DATE]",
            "",
            "Flag reference:",
            ...
            "  - Your current directory determines the local repo; use cd to change context",
            "  - Flag -bb is mutually exclusive with -lb and -rb",
            "  - Branch flags accept BRANCH or COMMIT id",
            "  - GLOB is a glob pattern: *.ext, file?.txt, **/*.py, *.{a,b}",
            "  - COMMIT is a hash code, such as that returned by status or log",
            "  - diff accepts 0, 1, or 2 sources, SOURCE can be local, remote, or a commit",
            "",
            "Get more help:",
    ...
    private static String commandHelp(String rawCommand) {
        ...
        return switch (command) {
            ...
            case "status" -> String.join("\n",
                ...
                "",
                "Notes:",
                "  - status -context shows where you are",
                "  - status -context URL shows what exists remotely",
            ...
            case "create" -> String.join("\n",
                header(),
                "",
                "create -- Create repo/branch, then switch (prints CONTEXT)",
                "",
                "Usage:",
            ...
            case "switch" -> String.join("\n",
                ...
                "  - Changing -rr and -rb does not change workspace files"
            );
            ...
            case "diff" -> String.join("\n",
                ...
                "",
                "Usage:",
                "  vgl diff [-v|-vv] [-all] [GLOB...]",
                "  vgl diff [-v|-vv] [-all] COMMIT [GLOB...]",
                "  vgl diff [-v|-vv] [-all] COMMIT1 COMMIT2 [GLOB...]",
                "  vgl diff [-v|-vv] [-all] [GLOB...] [-lr DIR]... [-lb BRANCH]... [-rr URL]... [-rb BRANCH]...",
                "",
                "How sources work:",
                "  - 0 sources: A = workspace; B = local branch (switch state)",
                "  - 1 source:  A = workspace; B = the specified source",
                "  - 2 sources: A and B are as specified",
                "",
                "Sources:",
                "  - The workspace is always a valid source.",
                "  - COMMIT is a commit identifier.",
                "  - Local sources use -lr and/or -lb (default repo/branch come from CWD/switch state).",
                "  - Remote sources use -rr and/or -rb (default URL/branch come from switch state).",
                "",
                "Examples:",
                "  vgl diff                            Workspace vs HEAD",
                "  vgl diff COMMIT                     Workspace vs commit",
                "  vgl diff -lb main -lb feature       Local branch comparison",
                "  vgl diff -rb dev                    Workspace vs remote 'dev' (remote URL from switch state)",
                "  vgl diff -rr URL1 -rb main -rr URL2 -rb main  Remote-to-remote",
                "",
                "Notes:",
                "  - Default: shows up to 10 matching changes/results; prints '  ...' if more exist",
                "  - Use '-all' to show all matching differences",
                "  - diff never changes files; it only reports differences"
            );
            case "log" -> String.join("\n",
                ...
                "",
                "Options:",
                "  -v              Show all commits (one-line summaries)",
                "  -vv             Show a full patch for the most recent commit or a specified COMMIT",
                "  -all            Show all commits (do not limit to 10 most recent)",
                "  -graph          Show ASCII graph of history",
                "",
                "Notes:",
                "  - Default output is a short, single-line summary per commit (message truncated to fit)",
                "  - Default: shows the 10 most recent commits and prints '  ...' if more exist; use '-all' to show all commits",
                "  - COMMIT accepts full or abbreviated commit ids",
                "  - DATE formats accepted: yyyy-MM-dd (day) or yyyy-MM (month)",
                "  - Ranges use two dots: COMMIT..COMMIT or DATE..DATE",
                "  - Multiple commits and/or date specs may be provided together",
                "  - Hint: run vgl log <commit> -vv for a specific commit patch"
            );
