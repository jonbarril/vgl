...
public class HelpCommand implements Command {
    ...
    private static String generalHelpVeryVerbose() {
        return String.join("\n",
            ...
            "    Use 'status' to observe context. Use 'switch' to show or set context.",
            "",
            "  Command guarantees",
            "    1) status: observe only; never changes workspace files",
            "    2) switch: sets context; -lb switches files; -rr/-rb never switch files",
            "    3) commit: saves workspace changes to local history only",
            "    4) checkout: creates a working copy of a remote repo",
            "    5) pull: reviews remote changes before applying; never auto-commits",
            "    6) push/checkin: send local commits to a remote",
            "    7) split/merge: local history operations only",
            "",
            "Typical workflows:",
            "",
            "  Working locally:",
