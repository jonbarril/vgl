# VGL: Use Case Bones ΓÇô Legacy, Alternate, and Future Concepts

## Concept Overview

The original advanced concept for VGL was to decouple the current working directory (cwd) from the current repository context. This enables workflows where the userΓÇÖs shell location and the repository context for VGL commands are independent, allowing for powerful multi-repo and cross-repo operations.
...
- **Command Behavior:**
  - All VGL commands operate relative to the current repo context, regardless of the userΓÇÖs cwd.
  - File arguments and globs are resolved relative to the userΓÇÖs actual cwd.
